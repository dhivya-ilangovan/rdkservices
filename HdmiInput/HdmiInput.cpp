...
namespace WPEFramework
{
    namespace Plugin
    {
	    ...
	    //version2 api end
            ...
            registerMethod(HDMIINPUT_METHOD_SUPPORTED_GAME_FEATURES, &HdmiInput::getSupportedGameFeaturesWrapper, this);
        ...
        void HdmiInput::hdmiInputALLMChange( int port , bool allm_mode)
        {
            ...
            allmModeStatus[port] = allm_mode;

            sendNotify(HDMIINPUT_EVENT_ON_ALLM_CHANGED, params);
        }

        uint32_t HdmiInput::getSupportedGameFeaturesWrapper(const JsonObject& parameters, JsonObject& response)
        {
            LOGINFOMETHOD();

            response["suppoortedGameFeatures"] = "ALLM";
            returnResponse(true);
        }
        ...
        uint32_t HdmiInput::getHdmiALLMStatusWrapper(const JsonObject& parameters, JsonObject& response)
        {
            ...
            bool allm = allmModeStatus[portId];
